...
import LabelText from 'qualificame-kiosk/app/components/LabelText';
...
describe('rendering', () => {
	let wrapper;
	const createWrapper = props => shallow(<LabelText {...props}/>);
	beforeEach(() => {
	...
	it('renders text with correct font size', () => {
		expect(wrapper.find(Text).props().style.find(p => p.fontSize).fontSize).toEqual(Fonts.size.label);
	});
